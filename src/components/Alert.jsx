...
const Alert = () => {
  ...
  return (
    ...
    >
      <div className="flex flex-col justify-center items-center bg-[#151c25] rounded-3xl border-2 border-cyan-600 min-w-min py-3 px-10">
        {alert.color == "red" ? (
          <FaRegTimesCircle className="text-red-600 text-4xl" />
        ) : (
          <BsCheck2Circle className="text-green-600 text-4xl" />
        )}
        <p className="text-white font-sans py-2">{alert.msg}</p>
      </div>
    </div>
  );
};
